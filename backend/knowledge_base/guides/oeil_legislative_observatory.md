...
- **Legacy URL, do not use**: `oeil.europarl.europa.eu/oeil/popups/ficheprocedure.do?reference=...` returns a 404 for recent procedure files. If any Brubru output or cached link uses this pattern, treat it as stale and rebuild it with the canonical pattern above.
...
Do not construct or trust links using the old `popups/ficheprocedure.do` path; it 404s for recent files and should never be presented to a user or embedded in a knowledge guide. If a URL built this way fails to resolve, do not guess a similar-looking path: fall back to the OEIL search page (`https://oeil.secure.europarl.europa.eu/oeil/en/search`) or to Brubru's own `_oeil_url()` builder in `backend/services/comparator/cell_extractors.py`, which is the single source of truth for how Brubru itself generates these links.
