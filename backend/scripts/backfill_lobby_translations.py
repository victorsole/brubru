...
def run_translate(limit: int, batch: int):
    ...
    """, (limit,))
    rows = cur.fetchall()
    print(f"[translate] {len(rows)} undetected items to process", flush=True)
    done = foreign = 0
    for r in rows:
        ...
        if lang not in SIX_SET and lang != "und":
            ...
            for tgt in SIX:
                ...
                """, (r["id"], tgt, t_title, t_sum, ENGINE))
        done += 1
        if done % batch == 0:
            conn.commit()
            print(f"  .. {done}/{len(rows)} ({foreign} foreign translated)", flush=True)
    conn.commit()
...
def run_images(limit: int, batch: int):
    ...
    """, (limit,))
    rows = cur.fetchall()
    print(f"[images] {len(rows)} items to probe for og:image", flush=True)
    found = 0
    ...
    with httpx.Client(timeout=12, follow_redirects=True, headers=headers) as client:
        for i, r in enumerate(rows, 1):
            ...
            try:
                ...
                img = ""  # mark checked; don't retry every run
            if img:
                found += 1
            # Empty string = "checked, none" so we skip it next run.
            cur.execute("UPDATE brussels_lobby_news_items SET image_url=%s WHERE id=%s",
                        (img, r["id"]))
            if i % batch == 0:
                conn.commit()
                print(f"  .. {i}/{len(rows)} ({found} with image)", flush=True)
            time.sleep(0.2)
    conn.commit()
    print(f"[images] done: {found}/{len(rows)} items got an image", flush=True)
    conn.close()
