"""
Backfill ``eu_cor_opinions`` from Cellar (metadata-only first pass).

Universe: every CELEX matching pattern ``5{YYYY}AR{NNNN}`` — the formal output
of the European Committee of the Regions. Cellar coverage is 3,617
opinions as of May 2026.

This is a METADATA-ONLY pass. Cellar serves the CoR catalogue but the
opinions' actual bodies live on cor.europa.eu (Drupal pages), not in Cellar
manifestations. A separate ``enrich_eu_eesc_bodies.py`` script handles the
body fetch from cor.europa.eu.
...
Run:
    python3.12 backend/scripts/backfill_eu_cor.py            # dry-run, 10 rows
    python3.12 backend/scripts/backfill_eu_cor.py --apply    # full backfill
"""
...
CELEX_PATTERN = re.compile(r"^5\d{4}AR[0-9R().]+$")
...
def classify_cor(title: Optional[str]) -> str:
    if not title:
        return "opinion"
...
# CoR commissions — most common abbreviations
_COR_COMMISSIONS = ["ECO", "INT", "REX", "NAT", "SOC", "TEN", "CCMI"]
_SECTION_PATTERNS = [
    (re.compile(rf"\b{s}/[\d-]+\b"), s) for s in _COR_COMMISSIONS
]
...
SELECT DISTINCT ?work ?celex ?date ?title ?resourceType WHERE {
  ...
  FILTER(REGEX(STR(?celex), "^5[0-9]{4}AR[0-9R().]+$"))
...
ON CONFLICT (celex) DO UPDATE SET
    work_uri = COALESCE(EXCLUDED.work_uri, eu_cor_opinions.work_uri),
    title = COALESCE(EXCLUDED.title, eu_cor_opinions.title),
    document_date = COALESCE(EXCLUDED.document_date, eu_cor_opinions.document_date),
    document_type = EXCLUDED.document_type,
    resource_type_uri = COALESCE(EXCLUDED.resource_type_uri, eu_cor_opinions.resource_type_uri),
    own_initiative = EXCLUDED.own_initiative,
    related_com_ref = COALESCE(EXCLUDED.related_com_ref, eu_cor_opinions.related_com_ref),
    related_celex = COALESCE(EXCLUDED.related_celex, eu_cor_opinions.related_celex),
...
def main():
    ...
    print("[INFO] Pulling CoR CELEX universe (5{YYYY}AE...) via Cellar SPARQL...", flush=True)
    universe = fetch_universe()
    print(f"[INFO] {len(universe):,} CoR opinions", flush=True)
