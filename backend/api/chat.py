...
def _as_uuid_or_new(value: Optional[str]) -> uuid_mod.UUID:
    """The caller's pre-generated message id, or a fresh one if it is unusable.

    Never raises. A malformed telemetry id must not stop the message being
    saved: the answer is the product, the join is the convenience. When this
    falls back, the analytics row keeps the id it was given and simply will not
    resolve, which the /users joinability check reports as an orphan rather
    than hiding.
    """
    if value:
        try:
            ...
            logger.warning("[chat] assistant_message_id %r is not a UUID; generated a new one", value)
...
def _save_messages(
    ...
    `assistant_message_id` is generated by the caller BEFORE streaming starts
    and handed to the analytics row too, so `chat_analytics.message_id` resolves
    to the message it describes. Before 11 September 2026 nothing wrote that
    column and every analytics row was an orphan.
    """
    db = SessionLocal()
    try:
        try:
        ...
        assistant_msg = Message(
            id=_as_uuid_or_new(assistant_message_id),
            chat_id=chat.id,
            role="assistant",
...
async def send_message(
    ...
    try:
        ...
        # The id this answer will be saved under. See stream_message.
...
async def stream_message(
    ...
    try:
        ...
        # The id this answer will be SAVED under, decided before a token is
        # generated. Analytics fires from inside chat_stream while the message
        # row is only written after the stream closes, so the id has to be
        # agreed up front; the alternative is a second write to correlate them
        # afterwards, which races and fails silently when a stream is abandoned.
        assistant_message_id = str(uuid_mod.uuid4())
