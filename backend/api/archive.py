...
# NOTE: these MUST be declared before the generic /{entity_type}/{track_id}
# routes, otherwise 'calendar' is parsed as an entity_type and 422s.
...
def archive_item(
    ...
    row = db.execute(
        text(f"SELECT id, archived_at FROM {tbl} WHERE id = CAST(:id AS uuid) AND user_id = :uid"),
    ...
    db.execute(
        text(f"UPDATE {tbl} SET archived_at = NOW(), archived_reason = :reason WHERE id = CAST(:id AS uuid)"),
        {"id": track_id, "reason": reason},
    )
    db.commit()

    refreshed = db.execute(
        text(f"SELECT archived_at, archived_reason FROM {tbl} WHERE id = CAST(:id AS uuid)"),
        {"id": track_id},
    ).fetchone()
...
def restore_item(
    ...
    tbl = _table_for(entity_type)
    row = db.execute(
        text(f"SELECT id FROM {tbl} WHERE id = CAST(:id AS uuid) AND user_id = :uid"),
        {"id": track_id, "uid": str(user.id)},
    ).fetchone()
    if not row:
        raise HTTPException(status_code=404, detail="Track not found or not owned by user")
    ...
    db.execute(
        text(f"UPDATE {tbl} SET archived_at = NULL, archived_reason = NULL WHERE id = CAST(:id AS uuid)"),
        {"id": track_id},
    )
    db.commit()
    return ArchiveResponse(
...
def list_archived(
    ...
    for et in types:
        ...
        for r in rows:
            out.append(ArchivedItem(track_id=str(r[0]), entity_type=et, archived_at=r[1], archived_reason=r[2] or "user"))
