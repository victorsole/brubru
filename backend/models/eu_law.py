...
class EULaw(Base):
    ...
    # CELEX-derived fields (parsed from celex string for fast filtering)
    celex_year = Column(Integer, index=True)  # e.g., 2024
    celex_type = Column(CHAR(1))  # R=Regulation, L=Directive, D=Decision, H=Recommendation
    celex_number = Column(Integer)  # e.g., 1689

    # Normalized document type (10 clean categories)
    doc_type_normalized = Column(String(50), index=True)

    # Corpus management
    corpus_version = Column(String(20), default='LEG_2025-11')  # Which corpus dump this came from
    ...
    def to_dict(self):
        ...
        return {
            ...
            'uuid': self.uuid,
            'celex': self.celex,
            'celex_year': self.celex_year,
            'celex_type': self.celex_type,
            'celex_number': self.celex_number,
            'doc_type': self.doc_type,
            'doc_type_normalized': self.doc_type_normalized,
            'title': self.title,
