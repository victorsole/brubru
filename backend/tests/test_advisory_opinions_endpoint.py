...
def test_both_committees_are_served(client, headers):
    ...
    everything = _get(client, headers, "/api/v2/advisory-opinions/all?limit=1")
    eesc = _get(client, headers, "/api/v2/advisory-opinions/eesc?limit=1")
    cor = _get(client, headers, "/api/v2/advisory-opinions/cor?limit=1")
...
def test_the_list_withholds_bodies_until_asked(client, headers):
    ...
    without = _get(client, headers, "/api/v2/advisory-opinions/all?limit=3")
    assert all(not item.get("body_txt") for item in without["data"])

    with_body = _get(client, headers, "/api/v2/advisory-opinions/all?limit=3&include_body=true&has_body=true")
...
def test_the_item_route_always_carries_the_whole_opinion(client, headers):
    listed = _get(client, headers, "/api/v2/advisory-opinions/eesc?limit=1&has_body=true")
    assert listed["data"], "no EESC opinion with a body"
    item = _get(client, headers, f"/api/v2/advisory-opinions/eesc/{listed['data'][0]['id']}")
...
def test_every_listed_item_carries_a_self_link_that_resolves(client, headers):
    listed = _get(client, headers, "/api/v2/advisory-opinions/all?limit=5")
    for item in listed["data"]:
        assert item.get("self"), f"no self link on {item.get('body_code')}/{item.get('id')}"
        assert f"/advisory-opinions/{item['body_code']}/{item['id']}" in item["self"]
    one = listed["data"][0]
    followed = _get(client, headers, f"/api/v2/advisory-opinions/{one['body_code']}/{one['id']}")
...
def test_the_envelope_can_serve_every_page_it_promises(client, headers):
    ...
    """
    limit = 100
    total = _get(client, headers, f"/api/v2/advisory-opinions/all?limit={limit}")["total"]
    last_page = (total + limit - 1) // limit
    remainder = total - (last_page - 1) * limit

    seen = set()
    for page in (1, 2, last_page - 1, last_page):
        payload = _get(client, headers, f"/api/v2/advisory-opinions/all?limit={limit}&page={page}")
    ...
    beyond = _get(client, headers, f"/api/v2/advisory-opinions/all?limit={limit}&page={last_page + 1}")
...
def test_the_body_filter_partitions_the_union(client, headers):
    everything = _get(client, headers, "/api/v2/advisory-opinions/all?limit=1")["total"]
    eesc = _get(client, headers, "/api/v2/advisory-opinions/all?body=eesc&limit=1")["total"]
    cor = _get(client, headers, "/api/v2/advisory-opinions/all?body=cor&limit=1")["total"]
...
def test_a_missing_opinion_is_a_404_with_a_reason_code(client, headers):
    response = client.get("/api/v2/advisory-opinions/cor/999999999", headers=headers)
...
def test_the_five_datapoints_are_present(client, headers):
    ...
    listed = _get(client, headers, "/api/v2/advisory-opinions/all?limit=50&has_body=true&include_body=true")
    rows = listed["data"]
...
def test_document_date_is_the_committees_date_not_our_capture_date(client, headers):
    ...
    listed = _get(client, headers, "/api/v2/advisory-opinions/all?limit=50&to=2020-12-31")
    for item in listed["data"]:
...
def test_every_declared_filter_actually_filters(client, headers):
    ...
    """
    unfiltered = _get(client, headers, "/api/v2/advisory-opinions/all?limit=1")["total"]
    ...
    for query, ok in cases.items():
        total = _get(client, headers, f"/api/v2/advisory-opinions/all?limit=1&{query}")["total"]
...
def test_q_matches_the_title_it_claims_to_match(client, headers):
    ...
    listed = _get(client, headers, "/api/v2/advisory-opinions/all?q=artificial+intelligence&limit=5")
