...
def _eu_laws_query() -> CorpusQuery:
    ...
    WHERE search_vector @@ websearch_to_tsquery('simple', %(phrase)s)
      ...
      AND ( %(lang_filter)s::TEXT[] IS NULL OR celex LIKE '3%' )
    ...
    def fmt(row, sub):
        celex = row['celex'] or ''
        title = (row['title'] or '')[:220]
        return {
            'notification_type': 'saved_search_alert',
            'title': f'{sub["label"]}: {celex}'.strip(),
            'message': title,
...
def _texts_adopted_query() -> CorpusQuery:
    ...
    def fmt(row, sub):
        ...
        return {
            'notification_type': 'saved_search_alert',
            'title': f'{sub["label"]}: {ta_ref}'.strip(),
            'message': (row['title'] or '')[:220],
...
def _rss_entries_query() -> CorpusQuery:
    ...
    def fmt(row, sub):
        return {
            'notification_type': 'saved_search_alert',
            'title': f'{sub["label"]}: {row["institution"] or "news"}',
            'message': (row['title'] or '')[:220],
            'action_url': row['link'],
