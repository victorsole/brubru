...
from .data_europa_scraper import DataEuropaScraper
...
class ScraperOrchestrator:
    ...
    async def get_eu_datasets(
        self,
        institution: str = "ep",
        limit: int = 20
    ) -> List[Any]:
        """
        Convenience method: Get EU institution datasets

        Args:
            institution: Institution code (ep, estat, consil, eea-sdi, etc.)
            limit: Maximum datasets

        Returns:
            List of datasets
        """
        scraper = self.scrapers['data_europa']
        return await scraper.get_eu_institution_datasets(institution, limit=limit)
    ...
    async def search_datasets(
        ...
        """
        Convenience method: Search open datasets
        ...
        scraper = self.scrapers['data_europa']
        if theme:
            return await scraper.search_by_theme(theme, limit=limit)
        else:
