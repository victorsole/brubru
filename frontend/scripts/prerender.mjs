...
async function prerender() {
  ...
  // sitemap.xml is now generated by scripts/generate_sitemap.mjs (runs after this).

  // Keep the clean SPA shell (empty #root + JS entry) as app.html. The
  // .htaccess SPA fallback serves THIS for client-only app routes (/main,
  // /my-eu-bubble, ...) instead of index.html. Serving the prerendered
  // homepage (index.html) to an app route makes React try to hydrate
  // homepage markup onto a different route -> hydration mismatch (React #418).
  // The clean shell has an empty #root, so main.tsx takes the createRoot path
  // (no hydration) and renders the real route fresh.
  try {
    const { renameSync } = await import('fs');
    renameSync(backupIndex, resolve(DIST, 'app.html'));
  } catch { /* no-op */ }
