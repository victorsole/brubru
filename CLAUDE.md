...
- **/morning Phase 3 cadence — DAILY core vs FRIDAY sweep (set 27 April 2026).** After the first full feature-tree population on 27 April, the data layers split clearly: **DAILY** = Chat (KB/triggers/system prompt) + My EU Bubble Calendar + Archive auto-sweep + passive My Files/Tracker via OEIL sync. **FRIDAY ONLY** = EU Law Comply matrix + Tenderator + Documents Generator templates + API page hero + Predictions/Position Analysis snapshot invalidation. **ON USER REQUEST ONLY** = Amendator example URL rotation, EC Public Consultations sync. Reason: Comply/Tenderator/Documents/API don't move enough to justify daily updates; Predictions/Position recompute on-demand anyway. Codified in `.claude/skills/morning/SKILL.md` Phase 3 (sub-phases 3D + 3F) and `.claude/skills/news/SKILL.md` Step 3 cadence table. Friday-only items approved on Mon-Thu /news runs queue to `memory/friday_sweep_queue.md`.
...
- **Negation paradox in guide WARNINGs (set 27 April 2026).** Never name a forbidden identifier inside its own "do not cite X" warning — that paradoxically RE-PRIMES the model to emit X. Incident: csam_regulation_online.md WARNING block listed "Birgit Sippel", "PE784.310", "PE784.377", "A10-0040/2026" inside the prohibition; chatbot kept regurgitating them despite the rule. Fix pattern: scrub all named identifiers from guide content (use category-only language: "rapporteur not yet verified per OEIL"); embed a verbatim refusal template in the system prompt that the model must use as the EXACT response. "Do not cite X" is insufficient when the model has X anchored in training data. See `services/ai_service.py` "NEVER FABRICATE RAPPORTEUR / PE / A / T IDENTIFIERS" section; memory `feedback_negation_paradox_in_warnings.md`.
