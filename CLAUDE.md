...
- **Verify any Commission meeting date / College announcement against the EC Transparency Register tentative-agenda PDF before asserting it anywhere (brief, LinkedIn, guide, calendar) — set 26 May 2026.** Trade press and a stale `college_agenda.md` are not enough. Two canonical search URLs + the proven Playwright→`api/files`→`pdftotext` retrieval recipe (the register HTML/JSON soft-blocks scripts; the highest `SEC(YYYY)NNNN` is the live agenda) are in `memory/feedback_ec_tentative_agenda_primary_source.md`. If the agenda's date column says "(tbc)", hedge — never assert hard adoption (the Tech Sovereignty Package slipped 25 Mar→15 Apr→27 May→3 June; nearly shipped a wrong-date LinkedIn post on 26 May before this check caught it).
...
- **lftp `--only-newer` silently skips files (set 4 May 2026).** Mandatory post-/siteground: curl `last-modified` on every changed user-visible HTML; force-push via `lftp put -O` if stale. Especially for deep-dives, `guides/index.html`, `data-architecture/index.html`. Full incident + force-push template: `memory/feedback_lftp_only_newer_skips.md`.
...
- **OEIL URL pattern (set 6 May 2026).** Legacy `oeil.secure.europarl.europa.eu/oeil/popups/ficheprocedure.do?reference=...` returns **404 for recent procedures** (verified live for 2024/0079(COD), 2026/0074(COD), 2022/0047(COD), 2026/0011(COD)). The working endpoint is `oeil.secure.europarl.europa.eu/oeil/en/procedure-file?reference=...`. Both URL-encoded `(` → `%28` and unencoded form work. Update any code that builds OEIL links — Comparator's `_oeil_url()` in `services/comparator/cell_extractors.py` is the canonical implementation. **Audit all places that built OEIL URLs in the past** — daily-brief verifier, knowledge guides, deep-dives, Position Analysis, Predictions — they may all be using the dead endpoint. **SWEEP COMPLETE 25 May 2026** (24 occurrences / 20 files: system prompt 3× + context_builder + real_time_enricher + scrape_eu_news + oeil_schemas + 12 guides + frontend widget + 2 deep-dives; verified end-to-end on prod). Lesson generalised in memory `feedback_oeil_url_endpoint.md`: when fixing a URL/string-template bug, grep ALL generation paths including the system prompt + guides, not just the obvious service.
...
- **Pack-objects SIGBUS bypass on `git push` (set 22 May 2026).** When `git push` returns `error: pack-objects died of signal 10` on this Mac (macOS provenance / mmap fault on `~/Documents/`-mounted repo), retry with `git -c pack.window=0 -c pack.depth=0 -c pack.compression=0 push origin main`. Disables delta computation + compression — the parts of pack-objects that crash. Larger pack on the wire but no SIGBUS. Disk pressure is a secondary factor, NOT the root cause (verified at 78% disk). Long-term: move repo out of `~/Documents/`. Companion memories: `feedback_git_push_sigbus_pack_objects`, `feedback_git_commit_hangs_use_plumbing`, `feedback_git_pack_integrity`.
