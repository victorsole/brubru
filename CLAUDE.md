...
**AI (Chat):** Free open-model chain. **NO ANTHROPIC** (removed 6 Aug 2026: too expensive; the open models are what Brubru runs on, and the provider class was deleted so it cannot return via an env var). `multi_provider_service.py` order = **Cerebras gpt-oss-120b (primary, streams, ~0.8s) → Gemini 2.5-flash (free, streams) → Groq qwen3.6-27b → NVIDIA llama-3.3-70b → Mistral → OpenAI (paid last resort)**. Groq decommissioned llama-3.3-70b on 16 Aug 2026; qwen3.6-27b replaced it (5/5 correct EU act numbers vs 3/5 for Groq's other suggestion gpt-oss-120b, which would also have duplicated the Cerebras primary). qwen wraps thinking in `<think>` — strip via `_strip_think()`, never a bare `<think>.*?</think>` regex, which misses the unclosed block a truncated answer leaves. Groq's 12K TPM still 413s on the full prompt; Mistral reads only ~30% of injected context so it sits last among the free tiers. Every SDK client sets `max_retries=0`: the chain IS the retry, and retrying inside one provider turned an instant Cerebras 429 into a 122-second wait, which was essentially all of Brubru's former 126-second latency (`memory/feedback_provider_chain_max_retries_zero.md`). Both the generator and the anti-hallucination validator run on this chain. `/api/chat/stream` is the ONLY path the UI calls; anything added to `chat()` alone ships dead (`memory/feedback_chat_stream_is_the_only_real_path.md`).
...
- **A fix on the branch is NOT a fix in production (28 July 2026).** Before claiming any fix is live: `git merge-base --is-ancestor <fix-sha> <deployed-tip>`, then a behavioural probe before/after. A green deploy log proves nothing. Incident: prod served 405 guides not 538 and applied zero post-processing to streamed answers for a week, because the deploy worktree branches off `origin/main` and drops feature-branch fixes. Memory: `feedback_deploy_tip_must_contain_the_fix`.
...
- **The repo lives at `~/Developer/brubru` (moved 3 Sep 2026).** `/Users/victorsole/Documents/GitHub/brubru` still exists on disk but is **stale** — never read it, write to it, or `cd` into it. `~/Documents` is iCloud-synced, and iCloud evicts file contents (`dataless` flag); memory-mapping an evicted file faults, which was the real cause of the `git push` SIGBUS and the multi-minute `git status`. After the move: `git status` 1.5s (was >180s), `pack-objects` clean. **Anything still resolving to the old path is a bug** — a wrong path never raises, it just silently uses the wrong tree. Detail: `project_repo_lives_in_developer_not_icloud`.
- **Pack-objects SIGBUS bypass on `git push` (set 22 May 2026, resolved 3 Sep 2026).** Superseded by the move above; keep only as recovery lore. If `git push` ever again returns `error: pack-objects died of signal 10`, retry with `git -c pack.window=0 -c pack.depth=0 -c pack.compression=0 push origin main` (disables delta+compression, the crashing parts) — but from the new path the iCloud explanation no longer applies, so re-diagnose rather than assume. Detail: `feedback_git_push_sigbus_pack_objects`, `feedback_git_commit_hangs_use_plumbing`.
- **Never hardcode an absolute `/Users/victorsole/...` repo path (set 3 Sep 2026).** Derive the root from the file's own location: `_REPO_ROOT = str(pathlib.Path(__file__).resolve().parents[2])` in `backend/scripts/*.py` and `backend/tests/*.py` (use this self-contained form, not a shared helper module — several scripts run as `python3.12 -m backend.scripts.<name>`, where `sys.path[0]` is the CWD); `BACKEND="${0:A:h:h}"` in zsh. The move stranded **75 hardcoded paths across 31 code files**, plus `.mcp.json` (the MCP server answered from the stale tree), two `.claude/settings.json` hooks, 35 skills including `/siteground` and `/railway`, and a live launchd job that deploys to SiteGround. Config files are the easy ones to miss — nothing compiles them. Detail: `feedback_no_hardcoded_absolute_repo_paths`.
