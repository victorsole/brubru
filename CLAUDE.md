...
- **On-disk presence is NOT deployability (set 14 September 2026).** A guide, asset or data file can exist locally, pass every content check and have live references pointing at it, while production cannot serve it because git does not track it. Two audits asked different questions and only one predicted prod: `audit_guide_triggers.py` globs the directory ("is it on disk?") and reported 0 orphans; `check_knowledge_integrity.py` shells out to git ("can prod have it?") and caught two guides with **32 live triggers between them** pointing at untracked files. It does not error: the ranker gates on `if guide_id in self.guides`, so the trigger fires, resolves to nothing and falls through to content matching, which reads as weak retrieval rather than a missing file. `git add` alone clears it (the script reads `git ls-files`, which counts the index). When two checks disagree, never average them: work out which question each asks. Memory: `feedback_on_disk_is_not_deployable`.
- **A client found it first: audit the whole chain (23 September 2026).** Joana (Terraqui) sent us a TRIS notification our paid DPP watch should have caught. Five silent links: feed frozen since March (hard-coded id), throttling read as "missing", watch never read TRIS, watch never ran daily (a MANDATORY line in a skill is a reminder unless the cron runs it), and `\b` is a BACKSPACE in PostgreSQL (use `\y`). Keep a per-client source ledger (`backend/data/client_sources/`, `scripts/client_source_ledger.py`) naming every source, who supplied it, a freshness test and the gaps. Memory: `feedback_client_found_it_first`.
- **Guide QUICK FACTS must be impossible to misread; trigger keys stay unique (16 September 2026).** The chat expanded "PfE" and "ESN" wrongly, called all five checked figures "above the record" when one bullet mixed two categories, and answered "unanimity" over a guide that said qualified majority. Spell out every acronym, give each category its own bullet with its count ("exactly two"), write "X, NOT Y" where the model may assume Y, then ask production via `/api/chat/stream`. `GUIDE_KEYWORD_TRIGGERS` was merged from 426 duplicate keys (250 lost guide links) to zero: extend an existing key in place and AST-assert zero duplicates after every edit. Memory: `feedback_quick_facts_must_be_unambiguous_to_the_model`, `feedback_trigger_dict_duplicate_keys_kill_edits`.
